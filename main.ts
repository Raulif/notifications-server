...
import { anyApi } from "npm:convex/server";
...
router.post("/new-subscription", async (ctx) => {
	try {
		...
		if (!subscription?.endpoint || !user) {
			...
			//   console.log(subscription);
			// Get existing users
			const storedUsers = ((await convex.query(anyApi.notifications.get, {})) ||
				[]) as Array<User>;

			const currentUser = storedUsers?.find((u) => u.user === user);
			if (currentUser) {
				// If user exist, update user subscription in DB
				await convex.mutation(anyApi.notifications.put, {
					id: currentUser._id,
					subscription,
				});
			} else {
				// If user does not exist, store new in DB
				await convex.mutation(anyApi.notifications.post, {
					user,
					subscription,
				});
			}
